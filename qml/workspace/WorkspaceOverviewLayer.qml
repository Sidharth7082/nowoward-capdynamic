...
Item {
    ...
    function moveSelection(delta) {
        if (delta === 1) {
            keyboardSelectedIndex = (keyboardSelectedIndex + 1) % workspacesShown;
        } else if (delta === -1) {
            keyboardSelectedIndex = (keyboardSelectedIndex - 1 + workspacesShown) % workspacesShown;
        } else if (delta === 5) {
            if (keyboardSelectedIndex < columns)
                keyboardSelectedIndex = Math.min(workspacesShown - 1, keyboardSelectedIndex + columns);
        } else if (delta === -5) {
            if (keyboardSelectedIndex >= columns)
                keyboardSelectedIndex = Math.max(0, keyboardSelectedIndex - columns);
        }
    }

    function selectWorkspaceIndex(idx) {
        if (idx >= 0 && idx < workspacesShown) {
    ...
    Keys.onReturnPressed: selectCurrentKeyboardWorkspace()
    Keys.onSpacePressed: selectCurrentKeyboardWorkspace()
    ...
    function normalizeToplevelAddress(tlv) {
        ...
        if (typeof tlv === "string") {
            ra = tlv;
        } else if (tlv.HyprlandToplevel && tlv.HyprlandToplevel.address !== undefined) {
            ra = String(tlv.HyprlandToplevel.address || "");
        } else if (tlv.address !== undefined) {
            ra = String(tlv.address || "");
        }
        ra = ra.trim().toLowerCase();
        if (ra === "" || ra === "0x" || ra === "0") return "";
        return ra.startsWith("0x") ? ra : "0x" + ra;
    }
    ...
    function refreshToplevels() {
        ...
        const ba = hyprlandData && hyprlandData.windowByAddress ? hyprlandData.windowByAddress : {}
        const wl = hyprlandData && hyprlandData.windowList ? hyprlandData.windowList : []
        const next = []; let sig = ""
        for (let i = 0; i < toplevelValues.length; i++) {
            ...
            let wd = ba[addr] || null
            if (!wd) {
                const appId = String(tlv.appId || "").toLowerCase()
                const title = String(tlv.title || "").toLowerCase()
                for (let j = 0; j < wl.length; j++) {
                    const w = wl[j]
                    if (!w) continue
                    const wClass = String(w.class || w.initialClass || "").toLowerCase()
                    const wTitle = String(w.title || w.initialTitle || "").toLowerCase()

                    if (wClass !== "" && appId !== "" && (appId.indexOf(wClass) !== -1 || wClass.indexOf(appId) !== -1)) {
                        wd = w
                        break
                    }
                    if (wTitle !== "" && title !== "" && (title.indexOf(wTitle) !== -1 || wTitle.indexOf(title) !== -1)) {
                        wd = w
                        break
                    }
                }
            ...
            if (wsId > start && wsId <= end) { next.push(tlv); sig += (addr || String(i))+"\x1e" }
        }
    ...
    Rectangle {
        ...
        implicitWidth: workspaceStage.implicitWidth
        implicitHeight: workspaceStage.implicitHeight
        color: "transparent"
        border.width: 0
                    ...
                    delegate: Row {
                        ...
                        Repeater {
                            ...
                            delegate: Rectangle {
                                ...
                                property bool isFocusedWorkspace: wsValue === root.effectiveActiveWorkspaceId
                                property bool isKeyboardSelected: wsValue === (root.keyboardSelectedIndex + 1)
                                ...
                                border.width: (isKeyboardSelected || isFocusedWorkspace || hoveredDrag) ? 2 : 1
                                border.color: isKeyboardSelected
                                    ? "#ff38bdf8"
                                    : (isFocusedWorkspace ? root.activeBorderColor : (hoveredDrag ? root.workspaceBorderHoverColor : StyleTokens.workspaceCellBorder))
                                clip: true
                                ...
                                ClippingRectangle {
                                    ...
                                    Image {
                                        anchors.fill: parent
                                        source: root.wallpaperPath !== "" ? encodeURI(root.wallpaperPath.startsWith("file://") ? root.wallpaperPath : ("file://" + root.wallpaperPath)) : ""
                                        fillMode: Image.PreserveAspectCrop
                                        autoTransform: true
                                        asynchronous: true; cache: true; opacity: 0.92
                                    }
                                    Rectangle {
                                        anchors.fill: parent
