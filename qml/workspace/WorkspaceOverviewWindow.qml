...
Item {
    ...
    readonly property real widthRatio: {
        ...
        return monitorWidth > 0 ? (widgetWidth * (monitorData.scale || 1)) / (monitorWidth * (widgetMonitor.scale || 1)) : 1;
    }
    readonly property real heightRatio: {
        ...
        return monitorHeight > 0 ? (widgetHeight * (monitorData.scale || 1)) / (monitorHeight * (widgetMonitor.scale || 1)) : 1;
    }
    ...
    readonly property real initX: {
        ...
        const position = positionOverride
            ...
            : (windowData.at ? windowData.at : [monitorData.x || 0, monitorData.y || 0]);
        return Math.max((position[0] - (monitorData.x || 0) - reserved[0]) * widthRatio * scale, 0) + xOffset;
    }
    readonly property real initY: {
        ...
        const position = positionOverride
            ? [positionOverride.x, positionOverride.y]
            : (windowData.at ? windowData.at : [monitorData.x || 0, monitorData.y || 0]);
        return Math.max((position[1] - (monitorData.y || 0) - reserved[1]) * heightRatio * scale, 0) + yOffset;
    }
